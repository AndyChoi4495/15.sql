/*
		...
		3. files QUery는 아래와 같다.
			...
			나. 업데이트라면 일단 k.substr(0, 1).toUpperCase() ? 'C' : 'U' 분기하고, 기존레코드가 있는지 체크한 후에 있다면
			기존 레코드의 status = '0' 파일도 옮긴다.
...
router.post('/', uploader.fields([{name: 'cover'}, {name: 'upfile'}]), async (req, res, next) => {
	...
	try {
		...
		if(req.files) {
			...
			fieldname = k.substr(0, 1).toUppperCase()
			if(isUpdate) {
				sql = " SELETE idx, savename FROM files WHERE fidx = ? AND fieldname=? AND status =? "
